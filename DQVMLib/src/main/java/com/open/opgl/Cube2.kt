...
class Cube2 : AppCompatActivity() {

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_triangle)
        initSurferView(surface,CubeRenderer())
    }
    ...
    inner class CubeRenderer : GLSurfaceView.Renderer {
        ...
        override fun onSurfaceChanged(gl: GL10?, width: Int, height: Int) {
            Log.d("DQ2020","onSurfaceChanged")
            GLES30.glViewport(0, 0, width, height)
            shape?.changed(height, width)
        }

        override fun onSurfaceCreated(gl: GL10?, config: EGLConfig?) {
            Log.d("DQ2020","onSurfaceCreated")
            shape = Shape()
        }
    }
    ...
    inner class Shape {
        ...
        fun changed(height: Int, width: Int) {
            opa(height,width,uMatrixLocation!!,mMatrix)
        }
    }
}